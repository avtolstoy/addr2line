...
class ElfFile {
  ...
  async parse() {
    ...
    try {
      ...
      throw new ElfFileError('Error while parsing elf file: ' + e.message);
    }
    return this;
  }
  ...
  async _readFile() {
    if (this._path) {
      this._data = await fsReadFile(this._path);
    } else if (this._data) {
      // Use the data already supplied to the constructor
    } else {
      throw new ElfFileError('No elf file path specified and no Buffer object supplied');
    }
    return this._data;
  }
...
function readULeb128(buf) {
  ...
  let size = 0;
  for (;;) {
    if (shift > 32) { // Values above 2^53 do not fit JS Number type
      throw new DwarfParserError('Read LEB128 value does not fit JS Number type');
    }
    const byte = buf.readUInt8(size);
    size += 1;
  ...
  for (;;) {
    ...
    if (!(byte & 0x80)) {
      if (shift < 32 && (byte & 0x40)) {
        result |= -1 << shift; // Sign extend the final byte
      }
      break;
    }
  }
  return {v: result, size: size};
};
...
function readAddress(size, buf, offset) {
  switch(size) {
    ...
    case 8: {
      ...
      if (hi > 0x1fffff) { // Values above 2^53 do not fit JS Number type
...
class DwarfLineNumberProgram {
  ...
  _addEntry() {
    this._program.decoded.push({...this._state});
  }
...
class DwarfParser {
  ...
  _parseAttributeData(attrib, cu, offset) {
    switch(attrib.form) {
      ...
      case 'DW_FORM_data2': {
        attrib.value = cu.data.readUInt16LE(offset);
        offset += 2;
        break;
      }
      case 'DW_FORM_block4': {
      ...
      case 'DW_FORM_data4': {
        attrib.value = cu.data.readUInt32LE(offset);
        offset += 4;
        break;
      }
      case 'DW_FORM_data8': {
        const lo = cu.data.readUInt32LE(offset);
        const hi = cu.data.readUInt32LE(offset + 4);
        // Constants such as DW_AT_const_value may hold values not representable in JS
        // Number; keep those as raw bytes. Values used as section offsets always fit
        if (hi > 0x1fffff && !(hi === 0xffffffff && lo === 0xffffffff)) {
          attrib.value = cu.data.slice(offset, offset + 8);
        } else if (hi === 0xffffffff && lo === 0xffffffff) {
          attrib.value = -1; // Common DW_AT_const_value convention for -1
        } else {
          attrib.value = hi * 0x100000000 + lo;
        }
        offset += 8;
        break;
      }
      ...
      case 'DW_FORM_sdata': {
        const val = readSLeb128(cu.data.slice(offset));
        attrib.value = val.v;
        offset += val.size;
        break;
      }
      ...
      case 'DW_FORM_ref8': {
        ...
        const inoffset = hi * 0x100000000 + lo;
        offset += 8;
        attrib.value = inoffset;
        attrib.reference = cu.data.slice(inoffset);
        break;
      ...
      case 'DW_FORM_ref_udata': {
        ...
        offset += val.size;
        attrib.value = val.v;
        attrib.reference = cu.data.slice(val.v);
        break;
      }
  ...
  _parseCUAttributes(cu) {
    ...
    while (offset < cu.data.length) {
      ...
      let die = {...abbrev, attributes: abbrev.attributes.map((attrib) => ({...attrib}))};
